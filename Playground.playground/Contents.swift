...
let messageList = [
	...
	"[WAIT|1", //Check that this does not impede New Game
	"[RESEARCH|JACK:\nI've got a guy on line 2. Seems urgent, but everyone else is busy right now. Should I patch you through?", //KEY
	"[RESPONSE|Go ahead|SYS|Nah|1",
	...
	"Great. Hopefully you can help me",
	"[RESPONSE|What's wrong?|1|Who are you?|1",
	"My name is Ben", //KEY
	"I originally came over to Pakistan on a business trip",
	"After my meetings were over I decided to take a few days off",
	"I love the mountains, so I went to a travel agency and hired a helicopter to take me up to Gilgit",
	"As we got cl--",
	"[SYS|Static",
	"[WAIT|2",
	...
	"[RESPONSE|Blizzard?|1|What happened?|Oh, so as we got closer, I asked him to take me on a quick pass over the Karakoram mountain range", //KEY
	"I'm fine for now",
	"I'm taking shelter in the helicopter wreckage",
	"Although it is starting to get a little chilly in here",
	"[RESPONSE|What happened?|1|Wreckage?|1",
	"Oh, so as we got closer, I asked him to take me on a quick pass over the Karakoram mountain range",
	"At first he resisted because the wind was starting to pick up",
	"But... it's Pakistan after all. There's not much you can't do with a little cash", // [space] after ...?
	"[WAIT|1.5",
	"That was a mistake though",
	"By the time we realized we were dealing with a full on blizzard, it was too late",
	...
	"I've had a little bit of time to get over the shock of the whole thing",
	...
	"Lucky this backback was in here",
	"This satellite phone, a few extra batteries, canned food, some water, and a compass",
	...
	"...and kind of to the east of the city",
	...
	"[RESEARCH|JACK:\nPickup right now is a no-go",
	"[RESEARCH|Local sources in the area are measuring wind speeds of up to sixty miles an hour",
	"[WAIT|1",
	...
	"[RESPONSE|Start walking|1|Ok, wait until morning|Yeah, someone would have to be pretty dumb to go out in this weather",
	...
	"The only difference it made is now my leg hurts even more.",
	...
	"Ah well. At least I'll have something to look at during my trek",
	...
	"[SYS|Ben is busy|0.05",
	"[RESEARCH|JACK:\nI'm sending you some photos to give you an idea of what the area looks like.",
	"[RESEARCH|IMAGE|Image1",
	"[WAIT|4",
	"[RESEARCH|IMAGE|Image2",
	"[WAIT|4",
	"[RESEARCH|IMAGE|Image3",
	"[WAIT|4",
	"[RESEARCH|Great spot for a vacation, but unfortunately for Ben it means a long, hard journey",
	"[SYS|Ben is busy|120",
	...
	"...but then again I haven't been able to see much of anything since the snow picked back up",
	...
	"[SKIP|SYS",
	"Ok, I guess I'll have to just grin and bear it.", //KEY
	...
	"Ok, north it is",
	"I'll let you know when I start climbing", //Redundant
	"[SKIP|SYS",
	
	"Ok, north it is", //KEY
	"It's a bit of a walk to the base of the mountain. I'll let you know when I start climbing",
	"[SYS|Ben is busy|20", //KEY
	"Ok, I've reached the base",
	"Wish me luck",
	"[SKIP|WAIT",
	...
	"...otherwise I'm going to be blindly stumbling down the mountain in the dark",
	"[SYS|Ben is busy|50",
	...
	"Oh well. I'm gonna have a bite to eat and then see if I can fall asleep", //Glitch?
	...
	"Wha-? What's going on?",
	"What was that?",
	"the moun-- the side of the mountains falling!",
	"...its all sliding towards me!",
	"[RESPONSE|It's an avalanche!|1|Run!|1",
	"already running",
	"wait. my backpack! i forgot to grab it!",
	"it has all my food",
	"and the compass!",
	"[RESPONSE|Go back for it|1|Forget it|but i need that stuff! i cant...",
	"i'd better be quick about it",
	"[SYS|Ben is busy|0.1",
	"got it! im heading back down!",
	"i don't think im gonna...",
	"[SYS|Connection Lost",
	"[GAMEOVER|Ben was buried by the avalanche|the moun-- the side of the mountains falling!",
	
	"but i need that stuff! i cant...", //KEY
	"[WAIT|1",
	"oh crap",
	"[WAIT|1",
	"im going",
	...
	"I managed to climb to higher ground and let the avalanche pass",
	"No sign of my stuff though. All I've got left is this phone.",
	...
	"[RESPONSE|I don't know|1|You'll make it!|Always the optimist",
	"Yeah, that makes two of us",
	"[SKIP|2", //Skip to message
	...
	"Great news!",
	"Just tell them to keep a lookout for a mountain. That's where I am",
	...
	"First I'm gonna have some breakfast. Catch you later",
	"[SYS|Ben is busy|8",
	"And off I go again",
	"That helicopter would be very welcome right now. Getting kind of tired of walking everywhere",
	"[SYS|Ben is busy|145",
	...
	"But I'm curious. I'll just get a little closer",
	...
	"The more I wander around in ignorance, the more I kind of wish I had trusted those people",
	...
	"...waking up to hot chai and bread, under the protection of a small tent... was heaven on earth",
	"[WAIT|1",
	...
	"[RESEARCH|JACK:\nI've contacted some people in Gilgit. They're sending a jeep to pick you up in Karimabad",
